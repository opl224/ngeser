...
import { Bell, MessageSquare, Trash2, X as XIcon, UserCheck, UserX } from 'lucide-react';
...
// Copied from AppSidebar / UserProfileDisplay - consider moving to a shared lib/notifications.ts
...
export function MobileHeaderIcons() {
 ...
 const handleAcceptFollowRequest = (requesterId: string, notificationId: string) => {
    ...
    if (!usersUpdateError) {
      ...
      toast({ title: "Permintaan Diterima", description: `Anda sekarang mengizinkan ${requesterUser.username} untuk mengikuti Anda.` });
    } else {
      toast({
  ...
  const handleDeclineFollowRequest = (requesterId: string, notificationId: string) => {
    ...
    toast({ title: "Permintaan Ditolak", description: `Anda menolak permintaan mengikuti dari ${requesterUser.username}.` });
  };


  if (!isClient || !currentUserId) {
    // Return a placeholder or null if not authenticated or not client-side yet
    return <div className="flex items-center gap-2"><div className="h-7 w-7"/><div className="h-7 w-7"/></div>;
  }
  ...
  return (
    <div className="flex items-center gap-1.5">
      <Link href="/dm" passHref>
        <Button variant="ghost" size="icon" className="relative h-9 w-9">
          ...
          {unreadMessagesCount > 0 && (
            <Badge variant="destructive" className="absolute -top-1 -right-1 h-4 w-4 p-0 text-[10px] flex items-center justify-center rounded-full">
              {unreadMessagesCount > 9 ? '9+' : unreadMessagesCount}
            </Badge>
          )}
          <span className="sr-only">Pesan Langsung</span>
        </Button>
      </Link>
      ...
      <DropdownMenu onOpenChange={handleOpenNotifications}>
        ...
        <DropdownMenuContent
            align="end"
            className="w-80 md:w-96 max-h-[calc(100vh-100px)] overflow-y-auto z-[60]"
